...
def Easygram(limits,song,bpm,barIn,barEnd,measure,unitSize,save=False):
    ...
    for i in range(unitStar,amountOfUnits):
        ...
        chunk_power = np.sum(np.abs(chunk))
        ...
        #energy.append(np.sum(fft)) # total energy in spectrum
        energy.append(np.max(fft)) # max energy point in spectrum
        #energy.append(chunk_power)
...
def GetNotesPeaks3D_Step(limits, song, bpm, barIn, barEnd, measure, unitSize, tr, save):
...
def GetNotesPeaks3D_Continuous(limits, song, bpm, barIn, barEnd, measure, unitSize, tr, save):
    multiband, energy, topFrequencies = Easygram(limits, song, bpm, barIn, barEnd, measure, unitSize, save)
    threshold = (max(energy)-min(energy))*tr + min(energy)
    x, y, z = [],[],[]
    for i in range(len(topFrequencies)):
